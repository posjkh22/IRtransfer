...
bool transferIRcode::InsertFunction(std::unique_ptr<Module> &m, LLVMContext &context, IRBuilder<> &builder){
	...
	//Value *helloWorld2 = builder2.CreateGlobalStringPtr("hello world!\n");
	...
	/* checker1 */
	Constant *checker1_ptr = m->getOrInsertFunction("checker1", funcType); 
	...
	/* checker2 */
	Constant *checker2_ptr = m->getOrInsertFunction("checker2", funcType); 
