...
Path *PathList::getNextPath(){
	...
	int num = 0;
	for(auto iter = pathList->begin();
			iter != pathList->end(); iter++, num++){
		if(num == Nextnum){
			returnPath = *iter;
			break;
		}
...
Path *PathList::createSameElementPathAll(Path* inpath, Path* outpath){

	Path *new_path = new Path; 
	Path *pass_path = nullptr;

	/*
	std::list<Path *> *pathList = getPathList();
	std::list<Path *>::iterator iter1;

	Path *prior_path = nullptr;
	for(iter1 = pathList->begin();
			iter1 != pathList->end(); iter1++){

		if(iter1 == pathList->begin()){

			prior_path = *iter1;
		}
		else{
			pass_path = createSameElementPath		
