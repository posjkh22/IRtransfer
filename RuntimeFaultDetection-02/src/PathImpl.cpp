...
Path *PathImpl::createSameElementPath(Path *path1, Path *path2){

	Path *new_path = new Path(path1);

	std::list<wBasicBlock *> *new_path_ptr 
		= new_path->getPath();
	
	std::list<wBasicBlock *> *path2_ptr 
		= path2->getPath();

	std::list<int> Not_same_flag_list;
	int same_flag = 0;
	...
	int i = 0;
	for(iter1 = new_path_ptr->begin();
			iter1 != new_path_ptr->end(); iter1++, i++){
	
		same_flag = 0;
		wBasicBlock *current = *iter1;

		for(iter2 = path2_ptr->begin();
				iter2 != path2_ptr->end(); iter2++){
			
			wBasicBlock *current2 = *iter2;
		...
		if(same_flag != 1){
			Not_same_flag_list.push_back(1);
		}else{
			Not_same_flag_list.push_back(0);
		}
	...
	i = 0;
	for(iter1 = new_path_ptr->begin(), iter3 = Not_same_flag_list.begin();
			iter1 != new_path_ptr->end(); iter1++, iter3++){
		
		int Not_same_flag = *iter3;
		if(Not_same_flag == 1){

			new_path_ptr->erase(iter1++);
		}
		else{
